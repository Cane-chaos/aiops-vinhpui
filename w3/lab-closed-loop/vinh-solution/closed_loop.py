...
def run_runbook(script: str, service: str, dry_run: bool, timeout_s: int = 30) -> bool:
    ...
    import os
    bash_exe = "C:/Program Files/Git/bin/bash.exe" if os.name == "nt" else "/bin/bash"
    cmd = [bash_exe, script, "--service", service]
    if dry_run:
        cmd.append("--dry-run")
...
def main():
    ...
    start_metrics_server()
    log.info("ORCHESTRATOR_START", service="all", action="init",
             config=args.config, dry_run=args.dry_run,
             poll_interval_s=cfg["poll_interval_seconds"])

    while True:
        if cb.is_open():
            log.error("CIRCUIT_BREAKER_HALT", service="all",
                      action="poll", result="halted",
        ...
        # Wait for all alert-processing threads to finish before next poll
        for t in threads:
            t.join(timeout=120)
