...
P1 deliverable: Train → Log → Register với alias 'production'.
...
def load_features(csv_path: str) -> pd.DataFrame:
    """Load CSV và trả về chỉ các feature columns, bỏ NaN."""
...
def train(
    ...
    """Train IsolationForest, log vào MLflow, register với alias 'production'."""
    ...
    with mlflow.start_run() as run:
        ...
        # --- Log scaler artifact (cần khi serve để scale input đúng) ---
        ...
        # --- Log model → tự động register vào MLflow Registry ---
        ...
        print(f"[pipeline] Rows trained: {len(X)}")
        print(f"[pipeline] Anomaly rate: {anomaly_rate:.4f}")

    # --- Gán alias 'production' cho version mới nhất ---
...
def main():
    parser = argparse.ArgumentParser(description="Train anomaly detection model (P1)")
    ...
    parser.add_argument("--n-estimators", type=int, default=100,
                        help="Number of trees in IsolationForest (default: 100)")
